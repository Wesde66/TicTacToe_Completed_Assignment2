...
public class Board {
	...
	// This variables are brought from the GameMain class
	...
	public boolean isDraw() {
		/*
		 * Here we run through the rows and columns and add them up. If there
		 * is an empty cell we return false as the game is not over yet. If there is
		 * no more empty cells we return true so game is a draw.
		 */
		for (int row = 0; row < GameMain.ROWS; ++row) {
			for (int col = 0; col < GameMain.COLS; ++col) {
	...
	//The below runs through condition statments. If one is met, it triggers true, if none are met triggers fales.
