...
public class GameMain extends JPanel implements MouseListener{
	...
	// This is done by creating a enumeration (same as player) with all the gamestates below.
	...
	public GameMain() {   
		...
		//We will be using the mouse to click. Therefore we use the mouseListener event
		...
		// Create the new instance of the Board so in the next step we can display it.
		board = new Board();
		
		//TODO: call the method to initialise the game board
		//We start the game here. Method at the botom, we are now just running it from here.
		initGame();
	}
	...
	public static void main(String[] args) {
		...
		javax.swing.SwingUtilities.invokeLater(new Runnable() {
	         public void run() {
				...
				//We now add the panel to the frame and run the GameMain method.
			    frame.getContentPane().add(new GameMain());
	...
	public void paintComponent(Graphics g) {
		...
		if (currentState == GameState.Playing) {          
			...
			if (currentPlayer == Player.Cross) {   
				...
				//Displays player X turn in red wrighting
				...
				//Displays player O turn in blue wrighting
				...
				statusBar.setForeground(Color.BLUE);//Changed to blue as when O wins its in his color.          
	  ...
	  /** Initialise the game-board contents and the current status of GameState and Player) */
		...
		public void updateGame(Player thePlayer, int row, int col) {
			...
			if(board.hasWon(thePlayer, row, col)) {
				...
				//Here we simply check to see how many O's or X' are in a column or row. 
				//Once they have the amount then the game state will change. If not it will continue to the else statment.
				currentState = (thePlayer == Player.Cross) ? GameState.Cross_won :GameState.Nought_won;
				
			} else 
				if (board.isDraw ()) {
					currentState = GameState.Draw;
					// If the above is not triggered then this will be set if condition is met.
	...
	public void mouseClicked(MouseEvent e) {  
		...
		//TODO: redraw the graphics on the UI  
		 // Simply call the repaint method
           repaint();
	}
